# grzyClothTool — RONNIE_T 汉化版

**_请注意，本工具仍处于「开发中」状态，你可能会遇到 Bug、功能缺失或体验问题。_**

> **汉化说明：** 本仓库为 grzyClothTool 简体中文汉化版本，由 **RONNIE_T** 翻译维护。  
> 原项目作者：[grzybeek](https://github.com/grzybeek/grzyClothTool)

编译汉化版请打开 `grzyClothTool.sln` 或使用 `dotnet publish`。

<p align="center">
  <img src="https://github.com/grzybeek/grzyClothTool/assets/40837847/30c72912-8828-4fa8-a84f-6f27a1f8eb5f">
</p>

**grzyClothTool** 是一款免费工具，用于轻松创建和管理 GTA5 附加包服装。  
现在你可以完成**几乎**所有其他同类工具能做的事，而且**完全免费**！

##

# 为什么选择 grzyClothTool？

- 独家功能
  - 轻松预览纹理
  - 轻松预览所有 Ped 配饰
  - 可同时预览多个 Drawable
  - 在 3D 预览中查看头发在帽子下的收缩效果
  - 在 3D 预览中查看高跟鞋所需的高度补偿
  - 无需担心单个附加包 128 件物品上限，工具会自动拆分为多个附加包！
- 开源
  - 无需担心运行来源不明的混淆 .exe，不知道它在后台做什么 😆
  - 免费使用，并将一直保持免费

# 致谢

- **[dexyfex](https://github.com/dexyfex/CodeWalker)** — 没有他和 CodeWalker，3D 预览功能无法实现！[在 Patreon 支持 dexyfex](https://www.patreon.com/dexyfex)
- [JagodaMods](https://discord.gg/jagoda) — 提供大量动力与创意 💖
- [ook](https://github.com/ook3d) — 修复与贡献

# 汉化

- **RONNIE_T** — 简体中文界面翻译
- QQ：3547376520
- KOOK：[https://kook.vip/XMaCDb](https://kook.vip/XMaCDb)

# 想表示感谢？

- 点击 **⭐ Star ⭐** 按钮
- 向更多人推荐这款工具！

# 捐赠

- 如果你在日常 Mod 制作中觉得这款工具有用，欢迎通过页面顶部的 [Sponsor](https://github.com/grzybeek/grzyClothTool?sponsor) 按钮捐赠，以支持原作者继续开发。

# 需要帮助？

- 加入 [KOOK](https://kook.vip/XMaCDb) 或 [Discord](https://discord.gg/HCQutNhxWt)

# 截图

![image](https://github.com/grzybeek/grzyClothTool/assets/40837847/5f568406-84cf-4050-aa4f-9dbd94066fca)
![image](https://github.com/grzybeek/grzyClothTool/assets/40837847/773b83ae-3609-4dbb-8cd2-c82ff67a00eb)
![image](https://github.com/grzybeek/grzyClothTool/assets/40837847/77bb0ff0-bd55-4895-846b-216cc9ce2349)
![image](https://github.com/grzybeek/grzyClothTool/assets/40837847/48ca84a6-1767-4a1c-b0a7-da8b51f8d2cd)
