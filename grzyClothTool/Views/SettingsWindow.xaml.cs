...
namespace grzyClothTool.Views
{
    ...
    { 
        ...
        {
            var title = e.Source.GetType().GetProperty("Title")?.GetValue(e.Source)?.ToString() ?? "选择主项目文件夹";
            ...
            if (selectedFolder.ShowDialog() == true)
            {
                try
                {
                    ...
                    LogHelper.Log($"主项目文件夹已更新为: {selectedFolder.FolderName}", LogType.Info);
                }
                catch (UnauthorizedAccessException)
                {
                    CustomMessageBox.Show(
                        "访问被拒绝。请选择一个您有写入权限的文件夹。",
                        "错误",
                        CustomMessageBox.CustomMessageBoxButtons.OKOnly,
