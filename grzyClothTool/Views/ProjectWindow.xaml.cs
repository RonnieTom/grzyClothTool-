...
namespace grzyClothTool.Views
{
    ...
    public partial class ProjectWindow : UserControl, INotifyPropertyChanged
    {
        ...
        private async void Add_DrawableFile(object sender, RoutedEventArgs e)
        {
            ...
            {
                Title = $"选择 Drawable 文件（{btn.Label}）",
            ...
            if (files.ShowDialog() == true)
            {
                ...
                LogHelper.Log($"正在扫描要添加的文件...", LogType.Info);

                await MainWindow.AddonManager.AddDrawables(files.FileNames, sexBtn);

                ProgressHelper.Stop("Drawable 添加完成，耗时 {0}", true);
        ...
        private async void Add_DrawableFolder(object sender, RoutedEventArgs e)
        {
            ...
            {
                Title = $"选择包含 Drawable 文件的文件夹（{btn.Tag}）",
                Multiselect = true
            };
            ...
            if (folder.ShowDialog() == true)
            {
                ...
                try
                {
                    ...
                    {
                        ProgressHelper.Stop("未找到 Drawable 文件", false);
                        return;
                    }

                    LogHelper.Log($"正在从 {folder.FolderNames.Length} 个文件夹添加 {allFiles.Length} 个 Drawable 文件...", LogType.Info);

                    await MainWindow.AddonManager.AddDrawables(allFiles, sexBtn);

                    ProgressHelper.Stop($"已添加 {allFiles.Length} 个 Drawable，耗时 {{0}}", true);
                    SaveHelper.SetUnsavedChanges(true);
                }
                catch (Exception ex)
                {
                    LogHelper.Log($"添加 Drawable 时出错: {ex.Message}", LogType.Error);
                    ProgressHelper.Stop("添加 Drawable 失败", false);
                }
            }
        }
        ...
        private void Delete_SelectedDrawable(object sender, RoutedEventArgs e)
        {
            ...
            if (count == 0)
            {
                CustomMessageBox.Show("未选中任何 Drawable", "删除 Drawable", CustomMessageBox.CustomMessageBoxButtons.OKOnly);
                return;
            }

            var message = count == 1
                ? $"确定要删除此 Drawable 吗？（{Addon.SelectedDrawable.Name}）"
                : $"确定要删除这 {count} 个选中的 Drawable 吗？";

            message += "\n这将更改此 Drawable 之后所有项的编号！\n\n是否改用预留槽位替换？";
            
            var result = CustomMessageBox.Show(message, "删除 Drawable", CustomMessageBox.CustomMessageBoxButtons.DeleteReplaceCancel);
        ...
        private void BuildResource_Btn(object sender, RoutedEventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("当前未加载项目。请先创建或打开项目。", 
                    "无项目", 
        ...
        private void DrawablesGroupBox_DragEnter(object sender, System.Windows.DragEventArgs e)
        {
            ...
            {
                LogHelper.Log($"DragEnter 事件中出错: {ex.Message}", LogType.Error);
                e.Effects = WpfDragDropEffects.None;
            }
        ...
        private void DrawablesGroupBox_DragOver(object sender, System.Windows.DragEventArgs e)
        {
            ...
            {
                LogHelper.Log($"DragOver 事件中出错: {ex.Message}", LogType.Error);
                e.Effects = WpfDragDropEffects.None;
            }
        ...
        private async void DrawablesGroupBox_Drop(object sender, System.Windows.DragEventArgs e)
        {
            try
            {
                ...
                else if (e.Data.GetDataPresent("FileGroupDescriptor") || e.Data.GetDataPresent("FileGroupDescriptorW"))
                {
                    ...
                    {
                        LogHelper.Log($"无法提取文件", LogType.Error);
                        e.Handled = true;
                        return;
                    }
                }
                ...
                if (undeterminedFiles.Count > 0)
                {
                    var result = CustomMessageBox.Show(
                        $"有 {undeterminedFiles.Count} 个 Drawable 无法自动判断性别，请为这些文件选择性别：",
                        "选择性别",
            ...
            {
                LogHelper.Log($"Drop 事件中出错: {ex.Message}", LogType.Error);
