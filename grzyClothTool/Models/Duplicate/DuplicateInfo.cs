        ...
        for (int i = 0; i < addons.Count; i++)
        {
            ...
            else if (item is Texture.GTexture texture)
            {
                foreach (var draw in addon.Drawables)
                {
                    ...
                    {
                        return $"附加包 {i + 1}：{draw.Name} → {texture.DisplayName}";
                    }
                }
            }
        }
