...
public class GTexture : INotifyPropertyChanged
{
        ...
        if (filePath != null)
        {
            try
            {
                ...
                Task<GTextureDetails?> _textureDetailsTask = LoadTextureDetailsWithConcurrencyControl(fullPath).ContinueWith(t =>
                {
                    if (t.IsFaulted)
                    {
                        LogHelper.Log($"加载贴图 '{DisplayName}' 详情失败: {t.Exception?.InnerException?.Message ?? t.Exception?.Message}", Views.LogType.Warning);
                        IsLoading = false;
                        return null;
                    }
            ...
            {
                LogHelper.Log($"无法加载贴图 '{DisplayName}': {ex.Message}", Views.LogType.Warning);
                IsLoading = false;
                IsPreviewDisabled = true;
            }
        }
    ...
    public async void LoadThumbnailAsync()
    {
        ...
        {
            LogHelper.Log($"找不到贴图 '{DisplayName}': {ex.Message}", Views.LogType.Warning);
            return;
        }
        ...
        await Task.Run(() =>
        {
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.Log($"无法为 {DisplayName} 生成图像缩略图");
            }
        });
    }
