...
public static class FileHelper
{
    ...
    public static string ResolveFilePath(string path)
    {
        ...
        {
            LogHelper.Log($"解析文件路径 '{path}' 时出错: {ex.Message}", Views.LogType.Warning);
            return path;
        }
    }
    ...
    public static async Task<GDrawable> CreateDrawableAsync(string filePath, Enums.SexType sex, bool isProp, int typeNumber, int countOfType)
    {
        ...
        {
            LogHelper.Log($"复制 Drawable 到项目资源失败: {ex.Message}。使用原始路径。", LogType.Warning);
        ...
        for (int txtNumber = 0; txtNumber < Math.Min(matchingTextures.Count, GlobalConstants.MAX_DRAWABLE_TEXTURES); txtNumber++)
        {
            ...
            {
                LogHelper.Log($"复制贴图到项目资源失败: {ex.Message}。使用原始路径。", LogType.Warning);
    ...
    public static void OpenFileLocation(string path)
    {
        ...
        {
            MessageBox.Show($"打开文件位置时发生错误：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
    ...
    public static async Task SaveTexturesAsync(List<GTexture> textures, string folderPath, string format)
    {
        ...
        var tasks = textures.Select(async texture =>
        {
            ...
            if (File.Exists(filePath))
            {
                LogHelper.Log($"无法保存贴图: {texture.DisplayName}。错误: 文件已存在。", LogType.Error);
                return;
            }
            ...
            if (fileExtension == ".ytd") 
            {
                ...
                catch (Exception ex)
                {
                    ...
                    LogHelper.Log($"无法保存贴图: {texture.DisplayName}。错误: {ex.Message}。", LogType.Error);
                } 
            }
            else
            {
        ...
        ProgressHelper.Stop($"已导出 {successfulExports} 个纹理，耗时 {{0}}", true);
    }

    public static async Task SaveDrawablesAsync(List<GDrawable> drawables, string folderPath)
    {
        Directory.CreateDirectory(folderPath);
        ProgressHelper.Start("开始导出 Drawable");
        ...
        var tasks = drawables.Select(async drawable =>
        {
            ...
            if (File.Exists(filePath))
            {
                LogHelper.Log($"无法保存 Drawable: {drawable.Name}。错误: 文件已存在。", LogType.Error);
                return;
            }
            ...
            catch (Exception ex)
            {
                ...
                LogHelper.Log($"无法保存 Drawable: {drawable.Name}。错误: {ex.Message}。", LogType.Error);
            }
        });

        await Task.WhenAll(tasks);

        ProgressHelper.Stop($"已导出 {successfulExports} 个 Drawable，耗时 {{0}}", true);
    }
}
