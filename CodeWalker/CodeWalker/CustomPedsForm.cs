    ...
    public partial class CustomPedsForm : Form, DXForm
    {
        ...
        public class ComponentComboItem
        {
            ...
            public string DrawableName
            {
                get
                {
                    return DrawableData?.GetDrawableName(AlternativeIndex) ?? "错误";
                }
            }
        ...
        public void RenderScene(DeviceContext context)
        {
            ...
            {
                pauserendering = true;
                UpdateStatus($"渲染错误 - 预览已暂停");
                LogError($"3D 预览渲染崩溃：{ex.Message}");
            }
        }
        ...
        private void ContentThread()
        {
            try
            {
                ...
                {
                    LogError("初始化游戏文件缓存失败：" + ex.Message);
                    LogError("GTA V 安装可能已损坏或不完整。3D 预览已禁用。");
                    UpdateStatus("错误：未找到或 GTA V 文件已损坏。3D 预览不可用。");
                    running = false;
                    return;
                }
            ...
            {
                LogError("ContentThread 中发生未处理的异常：" + ex.Message);
        ...
        private void LoadWorld()
        {
            try
            {
                UpdateStatus("正在加载 timecycle...");
                timecycle.Init(GameFileCache, UpdateStatus);
                timecycle.SetTime(Renderer.timeofday);
            }
            catch (Exception ex)
            {
                UpdateStatus("警告：Timecycle 加载失败");
                LogError($"Timecycle 初始化错误：{ex.Message}");
            }
            ...
            {
                UpdateStatus("警告：材质加载失败");
                LogError($"材质初始化错误：{ex.Message}");
            }
            ...
            {
                UpdateStatus("警告：天气加载失败");
                LogError($"天气初始化错误：{ex.Message}");
            }
            ...
            {
                UpdateStatus("警告：云层加载失败");
                LogError($"云层初始化错误：{ex.Message}");
            }
        ...
        private void UpdateModelsUI()
        {
            ...
            {
                AddDrawableTreeNode(drawable, $"已选 ({drawable.Name})", true);
            }
            ...
            if (SavedDrawables.Count > 0)
            {
                foreach (var drawable in SavedDrawables.Values)
                {
                    ...
                    AddDrawableTreeNode(drawable, $"已保存 ({drawable.Name})", true);
                }
            }
        }
        ...
        private void LiveTexturePreview_Click(object sender, EventArgs e)
        {
            ...
            liveTxtButton.Text = Renderer.LiveTextureEnabled ? "禁用" : "启用";
        ...
        private void AddCustomAnimButton_Click(object sender, EventArgs e)
        {
            ...
            {
                ofd.Filter = "YCD 文件 (*.ycd)|*.ycd|所有文件 (*.*)|*.*";
                ofd.Title = "选择动画文件";
                ...
                foreach (string filePath in ofd.FileNames)
                {
                    ...
                    try
                    {
                        ...
                        if (ycd.ClipDictionary != null && ycd.ClipMapEntries != null && ycd.ClipMapEntries.Length > 0)
                        {
                            ...
                            UpdateStatus($"已加载自定义动画：{fileName}  ({ycd.ClipMapEntries.Length} 个片段)");
                        }
                        else
                        {
                            MessageBox.Show($"无效的动画文件：{fileName}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"加载文件 {fileName} 时出错：\n{ex.Message}",
                            "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
